...
|------------|------|
| `ESC` | 退出程序 |
| `P` | PassiveMode（阻尼保护模式） |
| `空格键` | FixedPose（位控复位） |
| `Shift + 1` 或 `小键盘 1` | LocoMode（稳定行走模式） |
| `Shift + 2` 或 `小键盘 2` | Dance（查尔斯顿舞蹈） |
| `Shift + 3` 或 `小键盘 3` | KungFu（武术动作） |
| `Shift + 4` 或 `小键盘 4` | Kick（踢腿动作） |
| `Shift + 5` 或 `小键盘 5` | KungFu2（训练失败的武术动作） |
| `Shift + W / S` | 前进 / 后退 |
| `Shift + A / D` | 左移 / 右移 |
| `Shift + Q / E` | 左转 / 右转 |
| `↑ / ↓ / ← / →` | 移动（WASD 的替代方案） |

> **注意**：键盘输入的移动速度指令会覆盖手柄的摇杆输入。

#### Xbox 手柄（摇杆）控制
| 按键 / 组合 | 功能 |
|------------|------|
| `SELECT` | 退出程序 |
| `L3` | PassiveMode（阻尼保护模式） |
| `START` | FixedPose（位控复位） |
| `R1 + A` | LocoMode（稳定行走模式） |
| `R1 + X` | Dance（查尔斯顿舞蹈） |
| `R1 + Y` | KungFu（武术动作）— 仅推荐仿真使用 |
| `L1 + Y` | KungFu2（训练失败的武术动作）— 仅推荐仿真使用 |
| `R1 + B` | Kick（踢腿动作）— 仅推荐仿真使用 |
| `左摇杆` | 控制移动方向（前进/后退/左右） |
| `右摇杆（水平）` | 控制旋转（偏航角） |
